...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const $output_box_2 = $('#translation_output_2');
    ...
    output_quill_2 = new Quill($output_box_2[0], output_quill_options);
    ...
    function translate_and_set_output(text, html, input_lang_code, output_lang_code) { // put output into output box
        ...
        // re-translate, so abort previous translation (if any)
        if (xhr_translate_2 !== null) {
            xhr_translate_2.abort();
            xhr_translate_2 = null;
        }
        ...
        if (input_quill.getText().trim().length === 0) {
            ...
            output_quill_2.setContents(new Delta(), 'silent');
            return
        }
        ...
        }).done(function (translated_text) {
            xhr_translate_2 = null;
            const delta = output_quill_2.clipboard.convert({text: translated_text});
            output_quill_2.setContents(delta, 'silent');
            $translate_alert.hide();
        }).fail(function (jqXHR, text_status, error_thrown) {
            xhr_translate_2 = null;
            if (jqXHR.status > 0) {
                const delta = output_quill_2.clipboard.convert({
    ...
    function update_input_font_size() {
        ...
        $output_box_2.css('fontSize', size_rem.toString() + 'rem');
    }
        ...
        if (input_quill.getText().trim().length > 0) {
            if (debounce_timeout) {
                ...
                if (!output_quill.getText().trim().endsWith('...')) {
                    ...
                    output_quill.setContents(delta, 'silent');
                    const delta_2 = output_quill_2.clipboard.convert({
                        text: output_quill_2.getText().slice(0, -1) + "..."
                    });
                    output_quill_2.setContents(delta_2, 'silent');
                }
            }
